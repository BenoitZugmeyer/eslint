...
function processText(text, configHelper, filename, fix) {
    ...
    processorContext = new ProcessorContext(filename, text, config.settings);

    for (var plugin in loadedPlugins) {
        if (loadedPlugins[plugin]) {
            var processors = loadedPlugins[plugin].processors;

            if (typeof processors === "function") {
                processor = processors(processorContext);
            } else if (processors && Object.keys(processors).indexOf(fileExtension) >= 0) {
                processor = processors[fileExtension];
            }
    ...
    if (processor) {
        ...
        // NOTE: this breaks backward compatibility. We need to find a solution to handle
        // both cases, maybe by changing the `preprocess` function name.
        var parsedBlocks = processor.preprocess(processorContext);
        ...
        // NOTE: this breaks backward compatibility. We need to find a solution to handle
        // both cases, maybe by changing the `postprocess` function name.
        messages = processor.postprocess(processorContext, unprocessedMessages);
